...
class MarketActivity : AppCompatActivity(), RewardedVideoAdListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        marketViewModel.getInitialAsset().observe(this, Observer { tv_initial_asset.text = "$" + it })
        marketViewModel.getInitialMonthly().observe(this, Observer { tv_initial_monthly.text = "$" + it })
        marketViewModel.getInitialSalaryRaise().observe(this, Observer { tv_initial_salary_raise.text = it.toString() + "%" })
        ...
        cl_upgrade_asset.setOnClickListener {
            ...
            if (marketViewModel.getStack().value!! >= ITEM_COST) marketViewModel.applyItemRaiseSetCash()
            else Toast.makeText(this, "스택이 부족합니다", Toast.LENGTH_LONG).show()
        }

        //초기월금 아이템 구매
        cl_upgrade_monthly.setOnClickListener {
            if (marketViewModel.getStack().value!! >= ITEM_COST) marketViewModel.applyItemRaiseSetMonthly()
            else Toast.makeText(this, "스택이 부족합니다", Toast.LENGTH_LONG).show()
        }

        //연봉상승률 아이템 구매
        cl_upgrade_salary_raise.setOnClickListener {
            if (marketViewModel.getStack().value!! >= ITEM_COST) marketViewModel.applyItemRaiseSetSalaryRaise()
            else Toast.makeText(this, "스택이 부족합니다", Toast.LENGTH_LONG).show()
        }
