...
class Dialog_negotiation_unsuccess(context: Context) {
    ...
    fun start(theme: Int){
        ...
        when(theme){
            1->tv_fail_message.text = "초기자산 협상에 실패하여 초기자산이 $"+ dec.format(SET_CASH_STEP[0])+"가 되었습니다"
            2->tv_fail_message.text = "월급 협상에 실패하여 월급이 $"+ dec.format(SET_MONTHLY_STEP[0])+"가 되었습니다"
            3->tv_fail_message.text = "연봉 인상률 협상에 실패하여 연봉 인상률이 "+ SET_SALARY_RAISE_STEP[0]+"%가 되었습니다"
